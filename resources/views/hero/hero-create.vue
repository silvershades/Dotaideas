...
export default {
  ...
  methods: {
    ...
    fetchHero() {
      console.log('/api/dota-hero/?hero=' + this.selectedHero);
      if (this.selectedHero !== '') {
        this.loading_hero = true;
        axios.get('/api/dota-hero/?hero=' + this.selectedHero)
            .then((response) => {
              let new_hero = response.data.data[0];
    ...
    onFileChangeHero: function (payload) {
      ...
      this.dhero.image_show = URL.createObjectURL(file);
    ...
    initiateAjaxPost: function (event) {
      ...
      axios.post('/api/hero-store', formData, {'content-type': 'multipart/form-data'})
          .then((response) => {
